    ...
    m_midi_in_channel(0)
{
    ...
    for (int y=0; y<6; y++)
    {
        ...
        for (int x=0; x<24; x++) // The actual frets
        {
            ...
            Fret* b = new Fret((distance1*c_global_pixel_scale)+95,(y+1)*c_global_fret_height+(y>=6?12*40:75),fret_W*c_global_pixel_scale,c_global_fret_height);
            b->color((Fl_Color)18);
            b->color2(FL_RED);
            b->when(FL_WHEN_CHANGED);
            b->callback((Fl_Callback*) fret_callback,this);
...
void Guitar::Timeout(void)
{
    ...
    if(Fl::event_button1() && Fl::event() == FL_DRAG)
    {
        ...
        else
        {
            ...
            {
                goto jump; // FIXME if still inside previous fret then don't change
            }
            else
            {
